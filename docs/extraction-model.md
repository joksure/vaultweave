...
`OfficialExtractor.extract()` returns an `ExtractionResult`; `vaultweave extract` writes it to
...
- Test fixtures are **synthetic** (modelled on the documented payloads), not recordings of a real workspace.
  Run `vaultweave extract` against a real one to validate shapes before relying on M1 output.
